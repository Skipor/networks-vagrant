...
if [ "$#" -ne 3 ]; then
    ...
    echo "Usage: sudo sh create_router.sh ИНТЕРФЕЙС_РОУТЕРА НОМЕР_РОУТЕРА КОЛИЧЕСТВО_РОУТЕРОВ"
    exit 1
fi
...
ospf6d_config() {
	...
	echo "!"
	echo "router ospf6"
	echo " router-id ${prefix4}.${current_vlan}.1"
	echo " interface ${router_interface}.${prev_vlan} area 0.0.0.0"
	echo " interface ${router_interface}.${next_vlan} area 0.0.0.0"
	echo " interface ${router_interface}.${current_vlan} area 0.0.0.0"
	echo "!"
	echo "interface ${router_interface}.${prev_vlan}"
	echo " ipv6 ospf6 hello-interval 5"
	echo " ipv6 ospf6 dead-interval 10"
	echo "interface ${router_interface}.${next_vlan}"
	echo " ipv6 ospf6 hello-interval 5"
	echo " ipv6 ospf6 dead-interval 10"
	echo "interface ${router_interface}.${current_vlan}"
	echo " ipv6 ospf6 hello-interval 5"
